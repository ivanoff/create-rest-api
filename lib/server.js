...
app.checkAccess = function (req, res, next) {
  ...
  jwt.verify(token, secret, function(err, decoded) {
    ...
    var p = req.params;
    if(p.login && p.login !== decoded.login) return req._error.ACCESS_DENIED('Login owner error');
    if(p.group && p.group !== decoded.group) return req._error.ACCESS_DENIED('Group owner error');
