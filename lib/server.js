...
app.registerModel = function (name, model) {
  ...
  function addRelations(name, field, l, id, type) {
    if (!name || !l) return;
  ...
  Object.keys(model).forEach(function (field) {
    addRelations(name, field, model[field].link, '_id');
    addRelations(model[field].link, '_id', name, field, model[field].type);
  });
};
